...
_py_get_callable_name(PyObject *callable, gchar *buf, gsize buf_len)
{
  ...
  if (name && _py_is_string(name))
    {
      g_strlcpy(buf, _py_get_string_value_as_utf8(name), buf_len);
    }
  else
    {
      PyErr_Clear();
...
_py_log_python_traceback_to_stderr_in_debug_mode(void)
{
  ...
  PyObject *print_exception;
  PyObject *res;
  PyObject *exc, *value, *tb;

  PyErr_Fetch(&exc, &value, &tb);

  /* NOTE: no reference returned */
  traceback_module = PyImport_AddModule("traceback");
  print_exception = PyObject_GetAttrString(traceback_module, "print_exception");
  res = PyObject_CallFunction(print_exception, "OOO", exc, value, tb);
  if (!res)
    PyErr_Clear();
...
_py_format_exception_text(gchar *buf, gsize buf_len)
{
  ...
  if (str && _py_is_string(str))
    {
      g_snprintf(buf, buf_len, "%s: %s", ((PyTypeObject *) exc)->tp_name, _py_get_string_value_as_utf8(str));
    }
  else
    {
      g_strlcpy(buf, "<unknown>", buf_len);
...
const gchar *
_py_get_string_value_as_utf8(PyObject *object)
{
